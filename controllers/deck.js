...
export const deckCreate = async (req, res) => {
  ...
  if (!subject)
    return res.status(400).json({ message: "The subject is empty!!" });

  const text =
    "INSERT INTO decks(subject, user_id) VALUES ($1, $2) RETURNING * ;";
...
export const deckRecieve = async (req, res) => {
  ...
  if (!userId)
    return res.status(401).json({ message: "The user is missing twinn!" });

  const client = await pool.connect();
  const text = "SELECT decks.*, COUNT(cards.card_id) AS card_count FROM decks LEFT JOIN cards ON cards.deck_id = decks.deck_id WHERE decks.user_id = $1 GROUP BY decks.deck_id";
  const value = [userId];
...
export const deleteDeck = async (req, res) => {
  ...
  if (!userId)
    return res.status(401).json({ message: "The user is missing man!!" });

  const client = await pool.connect();
  const text =
    "DELETE FROM decks WHERE user_id = $1 AND deck_id = $2 RETURNING *;";
